/*
 # Rules:
   ...
   - Additional rules:
     ...
     - should not be any of these simple passwords (user123, password123, admin123, etc..)
...
/* 
  ...
  # Additional code:
    ...
    for( let i = 0; i < pwd.length; i++) {  // a. Iterate through each character of the password string
      // b. Check whether the character is = any of the characters in digits
      if(digit.includes(pwd[i])) {  // c. If we find a match, we can stop the loop and return true
    ...
    if(!containsDigit) { 
      console.log("Atleast one number  ⚠️") // c. If we find a match, we can stop the loop and return true
  }  
*/